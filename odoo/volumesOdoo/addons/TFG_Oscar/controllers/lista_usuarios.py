...
class ListaProductos(http.Controller):
    ...
    @http.route('/gestion/cargamento/usuarios', auth='public', cors='*', type='http')
    def obtenerDatosProductos(self, **kw):
        # Obtenemos la referencia del modelo (pensado en este programa para ser "productos")
        productos = request.env['usuario'].sudo().search([])

        #Generamos la lista de cargamentos
        lista_productos=[]
        for s in productos:
            lista_productos.append({'id':s.id,'email':s.email,'contrasena':s.contrasena,})
    ...
    @http.route('/gestion/apirest/clientes', auth="none", cors='*', csrf=False,
    ...
    def apiPost(self, **args):
        ...
        if (http.request.httprequest.method == 'POST'):
            ...
            record = request.env['clientes'].sudo().create(
    ...
    def apiGet(self, **args):
        ...
        if (http.request.httprequest.method == 'GET'):
            ...
            for s in record:
                ...
                'email':s.email,'tlfno':s.tlfno,'dni':s.cif,'img':img2})
                return http.Response( 
