...
class ListaProductos(http.Controller):
    ...
    @http.route('/gestion/cargamento/proveedores', auth='public', cors='*', type='http')
    def obtenerDatosProductos(self, **kw):
        # Obtenemos la referencia del modelo (pensado en este programa para ser "productos")
        productos = request.env['proveedores'].sudo().search([])
        ...
        for s in productos:
            ...
            'ciudad':s.ciudad,'email':s.email,'tlfno':s.tlfno,'cif':s.cif,'img':img2})
    ...
    @http.route('/gestion/apirest/proveedores', auth="none", cors='*', csrf=False,
    ...
    def apiPost(self, **args):
        ...
        if (http.request.httprequest.method == 'POST'):
            ...
            record = request.env['proveedores'].sudo().create(
