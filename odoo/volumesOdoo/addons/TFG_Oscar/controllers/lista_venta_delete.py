...
class EliminarEmpleados(http.Controller):
    ...
    @http.route('/gestion/apirest/delete/venta', auth="none", cors='*', csrf=False, methods=["GET"],
                type='http')
    def apiGet(self, **args):
        ...
        if (http.request.httprequest.method == 'GET'):
            record = http.request.env[ 'ventar'].sudo().search(search)
