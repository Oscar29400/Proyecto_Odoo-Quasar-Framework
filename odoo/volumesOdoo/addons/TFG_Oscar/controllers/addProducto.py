...
class ListaProductos(http.Controller):
    ...
    @http.route('/gestion/addProducto/<nombre>/<descripcion>/<precioCoste>', auth="none", cors='*', type='http')
    def apiPost(self,nombre,descripcion,precioCoste ,**args):
        productos = request.env['productos'].sudo().search([])

        #Generamos la lista de cargamentos
        record = request.env['productos'].sudo().create({'nombre':nombre,'descripcion':descripcion,'precioCoste':precioCoste})
